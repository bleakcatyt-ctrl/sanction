...
```bash
npm test             # 33 e2e-теста на изолированной базе: покупка → ключ → лоадер → отзыв → истечение
npm run smoke        # 40 проверок против уже запущенного сервера (по умолчанию localhost:3000)
```
...
```bash
npm test                                    # изолированная база в data/test
SMOKE_URL=https://sanction.example.com npm run smoke   # живой контур после деплоя
```
...
`npm run smoke` (`scripts/smoke.js`) идёт по тому же пути, но по HTTP против работающего
сервера: страницы, регистрация, песочная покупка, выдача ключа и пароля, полное рукопожатие
лоадера с проверкой ECDSA-подписи токена, heartbeat, активация при первом входе, гейт
админки, генерация и отзыв ключа, CSP/CSRF. Создаёт одного тестового пользователя и два
ключа — удобно прогнать после деплоя и перед сдачей.
